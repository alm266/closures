...
(define (range-check row numrows col numcols)
	(if (eq? #t (and (>= row 0) (< row numrows) (>= col 0) (< col numcols)))
		#t
		#f)
  )
...
(define (append-cols pattern1 pattern2)
  (make-pattern (give-greater (pattern-numrows pattern1) (pattern-numrows pattern2))
                (+ (pattern-numcols pattern1) (pattern-numcols pattern2))
                (lambda (row col) (if (> (pattern-numcols pattern1) col)
...
(define (append-rows pattern1 pattern2)
  (make-pattern (+ (pattern-numrows pattern1) (pattern-numrows pattern2))
                (give-greater (pattern-numcols pattern1) (pattern-numcols pattern2))
                (lambda (row col) (if (> (pattern-numrows pattern1) row)
                                      ((pattern-fn pattern1) row col)
                                      ((pattern-fn pattern2) (- row (pattern-numcols pattern1)) col))))
  )
  
;;; returns the larger value
;;; used in append-cols and append-rows
  (define (give-greater num1 num2)
  (if (<= num1 num2)
      num1
      num2
      ))
...
(define (flip-cols pattern)
  (make-pattern (pattern-numrows pattern)
                ...
                (lambda (row col) ((pattern-fn pattern)
                  row (- (pattern-numcols pattern) (+ col 1)))))
  )
...
(define (flip-rows pattern)
  (make-pattern (pattern-numrows pattern)
                (pattern-numcols pattern)
                (lambda (row col) ((pattern-fn pattern)
                  (- (pattern-numrows pattern) (+ row 1)) col)))
